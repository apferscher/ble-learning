...
static void sc_dhkey_check(struct smp_conn *conn)
{
	...
	if (conn->out)
	{
		...
		printf("sc dhkey check: conn->out = true\n");
		...
		for (i = 0; i < x; i++)
		{
			printf("%02X", conn->prnd[i]);
		}
		printf("\n");
		printf("conn->rrnd: ");
		for (i = 0; i < x; i++)
		{
			printf("%02X", conn->rrnd[i]);
		}
		printf("\n");
		printf("conn->mackey: ");
		for (i = 0; i < x; i++)
		{
			printf("%02X", conn->mackey[i]);
		}
		printf("\n");

		int y = 64;
		printf("conn->local_pk: ");
		for (i = 0; i < y; i++)
		{
			printf("%02X", conn->local_pk[i]);
		}
		printf("\n");
		printf("conn->remote_pk: ");
		for (i = 0; i < y; i++)
		{
			printf("%02X", conn->remote_pk[i]);
		}
		printf("\n");
		int z = 3;
		printf("io_cap: ");
...
{
	// printf("sc passkey send confirm\n");
